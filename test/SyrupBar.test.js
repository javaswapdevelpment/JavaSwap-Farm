...
contract('SyrupBar', ([alice, bob, carol, dev, minter]) => {
  ...
  it('safeJavaTransfer', async () => {
    assert.equal(
      (await this.java.balanceOf(this.syrup.address)).toString(),
      '0'
    );
    await this.java.mint(this.syrup.address, 1000, { from: minter });
    await this.syrup.safeJavaTransfer(bob, 200, { from: minter });
    assert.equal((await this.java.balanceOf(bob)).toString(), '200');
    assert.equal(
      (await this.java.balanceOf(this.syrup.address)).toString(),
      '800'
    );
    await this.syrup.safeJavaTransfer(bob, 2000, { from: minter });
    assert.equal((await this.java.balanceOf(bob)).toString(), '1000');
  });
});
